...
func Run(
	...
	if strings.TrimSpace(binary) == "" {
		return errors.New("hackrf_transfer binary is required")
	}
	...
	if err := command.Run(); err != nil {
		...
		if detail != "" {
			return fmt.Errorf("hackrf_transfer failed: %w: %s", err, detail)
		}
		return fmt.Errorf("hackrf_transfer failed: %w", err)
	}
